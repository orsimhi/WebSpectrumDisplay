...
def generate_sample_data(session, num_scans=100):
    ...
    print(f"Generating {num_scans} sample RF scans...")
    ...
    for i in range(num_scans):
        ...
        if random.random() < 0.3:
            ...
            try:
                ...
                print(f"Warning: Could not create markers for scan {i}: {e}")
        
        # Commit every 10 scans to avoid memory issues
        if (i + 1) % 10 == 0:
            session.commit()
            print(f"Created {i + 1}/{num_scans} scans...")
    
    # Final commit
    session.commit()
    print(f"✓ Created {scans_created} RF scans and {markers_created} markers")

def main():
    """Main function"""
    print("RF Spectrum Data Generator for TimescaleDB")
    print("=" * 50)
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print("Invalid number of scans. Using default: 200")
    ...
    try:
        ...
        print("You can now run the RF Spectrum Analyzer application.")
